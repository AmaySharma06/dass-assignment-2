...
class Game:
    ...
    def _handle_property_tile(self, player, prop):
        ...
        if prop.owner is None:
            print(f"  {prop.name} is unowned — asking price ${prop.economics["price"]}.")
    ...
    def buy_property(self, player, prop):
        ...
        self.bank.collect(prop.economics["price"])
        print(f"  {player.name} purchased {prop.name} for ${prop.economics["price"]}.")
        return True
    ...
    def auction_property(self, prop):
        ...
        print(f"\n  [Auction] Bidding on {prop.name} (listed at ${prop.economics["price"]})")
        highest_bid = 0
        highest_bidder = None
    ...
    def _handle_jail_turn(self, player):
        ...
        print(f"  {player.name} is in jail (turn {player.jail_info["jail_turns"] + 1}/3).")

        # Use a Get Out of Jail Free card if available
        if player.jail_info["get_out_of_jail_cards"] > 0:
            if ui.confirm("  Use your Get Out of Jail Free card? (y/n): "):
                player.jail_info["get_out_of_jail_cards"] -= 1
                player.jail_info["in_jail"] = False
                player.jail_info["jail_turns"] = 0
        ...
        # No action
        # Serve the turn
        player.jail_info["jail_turns"] += 1
        if player.jail_info["jail_turns"] >= 3:
    ...
    def _apply_card(self, player, card):
        ...
        if action in dispatcher:
            dispatcher[action](player, value)
